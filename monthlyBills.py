...
#plot graph
def plot(selection, screen, monthlyTracker_df, categories):
    if (selection.get() == ""):
...
########################################################################################################################################################################################
    ...
    selectButton = Button(monthlyGUI, text='Select Date', width = 15, height = 3, bg="#C0392B", highlightbackground="#C0392B", fg = "white",
                            command=lambda: plot(selection, monthlyGUI, monthlyTracker_df, categories)).pack(side=LEFT, anchor=W, expand=True)
    ...
    addExpenditureButton = Button(monthlyGUI, text="Add New Expenditure", width = 15, height = 3, bg="#C0392B", highlightbackground="#C0392B", fg = "white",
                                  command=lambda: addExpenditure(monthlyGUI, offlineMode, username)).pack(side=RIGHT, anchor=E, expand=True)
